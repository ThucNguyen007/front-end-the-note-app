...
const TodosList = props => {
  ...
  /*
  The useEffect hook is first called after the component renders
  So if we want to tell the component to perform some code after rendering
  */
  /*
  if the 2nd argument of useEffect contains an array of variables 
  and any of these variables change, useEffect will be called.
  useEffect with an empty array in its second argument gets called 
  only the first time the component renders.
  */
  useEffect(() => {
    retrieveTodos();
  }, [props.token]);
  // retrieveTodos calls TodoDataService.getAll() 
  const retrieveTodos = () => {
