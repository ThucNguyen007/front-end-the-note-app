...
const AddTodo = props => {
  // set to false means adding a todo list, and set to true when 
  // the component is in Editing mode
  let editing = false;
  let initialTodoTitle = "";
  let initialTodoMemo = "";

  // check if a state is passed into AddTodo. 
  // in todos-list.js, passing in a state in the link to edit
  ...
  //  onChangeTitle keeps track of the user-entered title value in the field
  ...
  /*
  saveTodo is called by the submit button’s onClick={saveTodo}. 
  In saveTodo, we first create a data object containing the todo’s properties
  */
  const saveTodo = () => {
    var data = {
      title: title,
    ...
    if (editing) {
      // routes to todobackend/api/views.py in our backend 
      // and calls TodoListCreate view to create the todo
